/*##################################
C++ 기초문법-C언어에서 확장된 문법(C++ 수업용)
파일명: 1.0.cppbasic.cpp
...
void HangManMain()
{
	string strComAnswer = "GAME";
	string strMyAnswer;
	char input;

	for (int i = 0; i < strComAnswer.length(); i++)	// 알파벳 갯수만큼 "_" 만들기
	{
		strMyAnswer.append("_");
	}

	cout << "####Hang Man Game####" << endl;

	for (int i = 0; strMyAnswer[i] != '\0'; i++)
		cout << strMyAnswer[i] << " ";
	cout << endl;
	while (1)
	{
		cout << "Input Alphabet >>> ";
		cin >> input;
		int index = strComAnswer.find(input, 0);
		//cout << "Input : " << input << ", " << index << endl;
		if (index == -1)	//예외처리
		{
			cout <<input<< "은(는) 없는 문자입니다." << endl;
			for (int i = 0; strMyAnswer[i] != '\0'; i++)
				cout << strMyAnswer[i] << " ";
			cout << endl;
		}
		...
		{
			input = strComAnswer[index];
			strMyAnswer[index] = input;
			for (int i = 0; strMyAnswer[i] != '\0'; i++)
				cout << strMyAnswer[i] << " ";
			cout << endl;
		}
		...
		if (strMyAnswer == strComAnswer)	//종료시점
		{
			cout << "정답입니다!" << endl;
			break;
		}
